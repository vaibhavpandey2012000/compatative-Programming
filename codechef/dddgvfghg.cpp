...
//ll distn2 = abs(b - q);
//ll dist3 = abs(r - p);
if ((b - a) != 0 && (q - p) % (b - a) == 0)slo = 1;
if ((c - a) != 0 && (r - p) % (c - a) == 0)slo = 1;
if ((b - c) != 0 && (q - r) % (b - c) == 0)slo = 1;
ll f = 0;
ll ma = 1;
ll del = 0;
if (p == q || q == r || r == p)ma = 0;
...
if (a != b && p - ((q - p) / (b - a))*a + c == r && (q - p) % (b - a) == 0)del = 1;
if (c != b && r - ((q - r) / (b - c))*c + a == p && (q - r) % (b - c) == 0)del = 1;
if (a != c && p - ((r - p) / (c - a))*a + b == q && (r - p) % (c - a) == 0)del = 1;
if (del == 1)res = 2;
ll x[3], y[3];
x[0] = a; y[0] = p;
x[1] = c; y[1] = r;
