		...
		int quary(int *tree,int ss,int se,int qs,int qe,int ind)
		{
			...
			if(ss>=qs && se<=qe)
				return tree[ind];
			int m=(ss+se)/2;
			int left=quary(tree,ss,m,qs,qe,2*ind);
			int right=quary(tree,m+1,se,qs,qe,2*ind+1);
			return min(left,right);
		}

		void build(int *arr,int *tree,int s,int e,int ind)
		{
			if(s==e)
			{
				tree[ind]=arr[s];
			...
			int mid=(s+e)/2;
			build(arr,tree,s,mid,2*ind);
			build(arr,tree,mid+1,e,2*ind+1);
			tree[ind]=min(tree[2*ind],tree[2*ind+1]);
			return ;
		}
		 ...
		 //   cin>>test;
		    while(test--){
		    	...
		    	while(q--)
		    	{
		    		...
		    		if(ii==1)
		    		{
		    			cout<<quary(tree,0,n-1,a-1,b-1,1)<<endl;
		    			continue;
		    		}
		    		 else if(ii==2)
		    		{
		    			update(tree,0,n-1,a,b,1);
		    			REP(i,1,10)
		    			cout<<tree[i]<<" ";
		    		}
		    	}
